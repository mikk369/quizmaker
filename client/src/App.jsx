...
const App = () => {
    const [teams, setTeams] = useState([{ name: '', score: 0, totalScore: 0 }, { name: '', score: 0, totalScore: 0 }, { name: '', score: 0, totalScore: 0 }]);
    const [quizStarted, setQuizStarted] = useState(false);

  const startQuiz = (teamNames) => {
    const initialTeams = teamNames.map((name) => ({ name, score: 0, totalScore: 0 }));
